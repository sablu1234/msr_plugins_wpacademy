 ...
 * Project meta information
 */
function msr_post_voting_butotns($attrs){
    $attrs = shortcode_atts(
		array(
			'like' => 'Like',
			'dislike' => 'Dislike',
    ...
    $html = '<div class="msr-voting-buttons" >';
        $html .=sprintf(
            '<button class="msr-like" data-post-id="%s" data-user-id="%s"  >%s</button>',
             esc_attr( $post_id ),
             esc_attr( $user_id ),
             esc_html( $attrs['like'] )
        );
        $html .=sprintf(
            '<button class="msr-dislike" data-post-id="%s" data-user-id="%s"  >%s</button>',
             esc_attr( $post_id ),
             esc_attr( $user_id ),
...
add_shortcode('VOTING_BUTTONS','msr_post_voting_butotns');